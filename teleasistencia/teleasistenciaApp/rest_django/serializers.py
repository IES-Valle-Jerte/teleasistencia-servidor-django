...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'url', 'database_id', 'is_active', 'last_login', 'username', 'first_name', 'last_name', 'email',
                  'date_joined', 'groups', 'imagen', 'paciente']
        depth = 1
...
class Tipo_Recurso_Comunitario_Serializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        return self.Meta.model.objects.db_manager(getDatabaseByUser(self.context["request"].user)).create(
            **validated_data)
...
class Desarrollador_Serializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Desarrollador
        fields = ['desarrollador_tecnologias', 'es_profesor', 'descripcion', 'imagen',
                  'nombre']  # Indica todos los campos
